...
run_sim <- function(model_config){
  ...
  dist_pre_op_ward_los <- function() rnorm(1, 60 * mc$pre_op_los, sd = 6) # x60 = hours
  #dist_pre_op_ward_los()

  dist_operating_time <- function() rnorm(1, mc$theatre_proc_length, sd = 6) # minutes
  #dist_operating_time()

  dist_post_op_ward_los <- function() rnorm(1, 60 * 24 * mc$post_op_los, sd = 6) # 60x24 = days
